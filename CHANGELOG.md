...
- **侧边栏会话分组可折叠**。「📌 已置顶」与「近期」两个分组标题现在可点击折叠，
  右侧显示条数，折叠状态持久化（`localStorage` 的 `weknora-sidebar-collapsed`）。
  会话很多时可以一键把列表收短，底部入口不用再靠删会话才能露出来。

- **设置页：命名连接，一键切换**。原先「连接 WeKnora」只有一对 baseUrl / API Key
  输入框，换一台服务器就得把地址和密钥重新敲一遍。现在：

  - 增加**连接名称**字段；点💾保存即把当前表单登记为一条命名连接
  - 新增「已保存的连接」列表，点任意一行即**切换**（写入该连接的 baseUrl / apiKey /
    默认 Agent），当前生效的显示「使用中」徽标
  - 每条连接可**编辑 / 删除**；「＋新建」清空表单另存一条
  - 首次升级会自动把现有配置登记成一条连接，列表不会一开始就是空的
  - 切换后如果**服务器地址真的变了**才重置当前会话（不同服务器上的会话互不相通）；
    同一台服务器上多条命名连接之间切换不会丢掉当前会话

  实现见 `src/lib/connections.ts`：App 其余部分仍然只读 `weknora-settings`，
  连接只是它的「配置档案」，因此不需要改动调用方。

### 修复

- **会话增多后，抽屉底部「知识库 / 设置」看不见（关键）**。根因是 **`.wrap` 与
  `.list` 两个类只写在 `sidebar.ts` 的 `UNUSED_styles` 里** —— 该组件用的是
  Light DOM，那段 shadow 样式**完全不生效**，而全局 `styles.css` 里又没有它们。

  于是 `.wrap` 不是 flex 容器、`.list` 也没有 `overflow-y: auto`：会话列表**没有
  滚动容器、无限向下撑开**，把 `.footer` 挤出抽屉，再被 `ll-sidebar` 的
  `overflow: hidden` 直接裁掉。用户看到的就是「会话一多，底部两个入口就没了，
  只能删会话才能露出来」。

  现在把缺失的规则补进全局（并限定在 `ll-sidebar` 内，避免 `.wrap` / `.list`
  这种通用类名影响别处），同时给顶部与底部部件加 `flex-shrink: 0`，保证永不被挤压。

  真机验证（注入 40 个会话复现）：列表 `overflow-y: auto`、可视 600px / 内容 2588px
  可滚动；把列表滚到底部时底部按钮位置纹丝不动（`bottom: 797` 不变，仍在可视区内）；
  折叠「近期」后条目 40 → 3、内容高度 2588 → 600，底部按钮仍可见。
